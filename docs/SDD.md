...
+Thu thập flow từ NetFlow v5/v9, Zeek/Suricata JSON, Syslog (CEF/LEEF), REST API               
+Chuẩn hóa dữ liệu về một schema thống nhất (Normalized Flow Record)                           
+Lưu trữ dữ liệu có phân vùng theo thời gian, tối ưu cho truy vấn IP                          
+REST API truy vấn: filter, aggregation (top talkers/ports), phân trang
+Đáp ứng các yêu cầu phi chức năng: hiệu năng, tin cậy, mở rộng, bảo mật, vận hành, bảo trì   
+Triển khai bằng Docker Compose và script sinh dữ liệu giả phục vụ demo                       
...
| **Zeek**       | Network security monitor sinh log dạng văn bản/JSON; conn.log chứa thông tin flow.                                                                                          |
